...
<div class="container">
    <div class="row justify-content-center align-items-center">
        <div class="col">
            <h2>Bienvenido Evaluador: <?php echo $_SESSION['usuario_nombres']; ?></h2>
            <p>Esta sección permite el aprobar los trabajos de grado del trabajo de grado</p>
            <p>Su perfil es: <button class="btn btn-primary">Evaluador</button></p>
        </div>
    </div>
</div>
        ...
        <table class="table table-primary">
            <thead>
                <tr>
                    ...
                    <th scope="col">Conclusión Evaluador</th>
            ...
            <tbody>
                <?php foreach ($trabajos as $trabajo): ?>
                    <tr>
                        <form method="POST" action="actualizar_estado_evaluador.php">
                            ...
                            <td>
                                <select name="estado_evaluador" class="form-select">
                                    <option value="Pendiente" <?= ($trabajo['estado_evaluador'] == 'Pendiente') ? 'selected' : '' ?>>Pendiente</option>
                                    <option value="Aprobado" <?= ($trabajo['estado_evaluador'] == 'Aprobado') ? 'selected' : '' ?>>Aprobado</option>
                                    <option value="Rechazado" <?= ($trabajo['estado_evaluador'] == 'Rechazado') ? 'selected' : '' ?>>Rechazado</option>
                                </select>
                            </td>
                            <td>
                                <textarea name="conclusion_evaluador" id="conclusion_evaluador"><?= htmlspecialchars($trabajo['conclusion_evaluador']) ?></textarea>
                            </td>
                            <td>
